...
class Account {
  ...
  createKeyAtIndex(branch, index, force) {
    let key;

    switch (branch) {
      case 0:
        if (index > this.receiveDepth && !force)
            throw new Error('Requested index exceeds current receive depth');
        if (index === -1)
          index = this.receiveDepth;
        key = this.deriveReceive(index);
        break;
      case 1:
        if (index > this.changeDepth && !force)
            throw new Error('Requested index exceeds current change depth');
        if (index === -1)
          index = this.changeDepth;
        key = this.deriveChange(index);
        break;
      case 2:
        if (index > this.nestedDepth && !force)
            throw new Error('Requested index exceeds current nested depth');
        if (index === -1)
          index = this.nestedDepth;
        key = this.deriveNested(index);
        break;
      default:
        throw new Error(`Bad branch: ${branch}.`);
    }
