  ...
  initRouter() {
   ...
   // Derive receive address
    this.get('/wallet/:id/address', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const acct = valid.str('account');
      const index = valid.u32('index', -1);
      const force = valid.bool('force', false);
      const addr = await req.wallet.deriveReceive(acct, index, force);
    ...
    this.get('/wallet/:id/change', async (req, res) => {
      ...
      const index = valid.u32('index', -1);
      const force = valid.bool('force', false);
      const addr = await req.wallet.deriveChange(acct, index, force);

      res.json(200, addr.toJSON(this.network));
    });

    // Derive nested address
    this.get('/wallet/:id/nested', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const acct = valid.str('account');
      const index = valid.u32('index', -1);
      const force = valid.bool('force', false);
      const addr = await req.wallet.deriveNested(acct, index, force);
