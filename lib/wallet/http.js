...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get('/wallet/:id/listaddresses/:account', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const acct = valid.str('account');
      const account = await req.wallet.getAccount(acct);
      ...
      const hashes = await req.wallet.getAccountHashes(account.accountIndex);
      let addresses = [];
      let addrVer;
      let addrType;

      // witness program 0 only, pubkeyhashonly (no scripthash)
      if (account.witness) {
        addrVer = 0;
        addrType = 2;
      } else {
      ...
      hashes.forEach( function(hash){ addresses.push(Address.fromHash(hash, this.type, this.version).toString(this.network)) }, scope);
