...
class Wallet extends EventEmitter {
  ...
  createNested(acct = 0) {
   ...
   /**
   * Derive a receiving address (doesn't increment receiveDepth).
   * @param {(Number|String)?} acct
   * @param {Number} index
   * @param {Boolean} force
   * @returns {Promise} - Returns {@link WalletKey}.
  ...
  deriveReceive(acct = 0, index, force) {
    return this.deriveKey(acct, 0, index, force);
  }

  /**
   * Derive a change address (doesn't increment receiveDepth).
   * @param {(Number|String)?} acct
   * @param {Number} index
   * @param {Boolean} force
   * @returns {Promise} - Returns {@link WalletKey}.
  ...
  deriveChange(acct = 0, index, force) {
    return this.deriveKey(acct, 1, index, force);
  }

  /**
   * Derive a nested address (doesn't increments receiveDepth).
   * @param {(Number|String)?} acct
   * @param {Number} index
   * @param {Boolean} force
   * @returns {Promise} - Returns {@link WalletKey}.
  ...
  deriveNested(acct = 0, index, force) {
    return this.deriveKey(acct, 2, index, force);
  }
  ...
  /**
   * Derive an address (doesn't increment depth)
   * @private
   * @param {(Number|String)?} acct
   * @param {Number} branch
   * @param {Number} index
  ...
  async deriveKey(acct, branch, index, force) {
    const account = await this.getAccount(acct);

    if (!account)
      throw new Error('Account not found.');

    return account.createKeyAtIndex(branch, index, force);
  }
