...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('signrawtransactionwithwallet', this.signrawtransactionwithwallet);
  }
  ...
  async importPrivKey(args, help) {
    ...
    const rescan = valid.bool(2, true);
  ...
  async signrawtransactionwithwallet(args, help) {
    if (help || args.length < 1 || args.length > 3) {
      throw new RPCError(errs.MISC_ERROR,
    ...
    // Get as many coins as we can from wallet itself
    tx.view = await this.wallet.getCoinView(tx);

    // Add coins from query, find wallet keys and add scripts to them
    const keys = [];
    if (prevout) {
      for (const prev of prevout) {
        ...
        const addr = coin.getAddress();
        const key = this.wallet.getKey(addr);
    ...
    // Worst-case scenario, we need to get coins from mempool/chain
    if (!tx.hasCoins()) {
      for (const input of tx.inputs) {
        if (!tx.view.hasEntry(input.prevout)) {
          const coin = await this.client.getCoin(input.prevout);

          if (!coin)
            throw new RPCError(errs.MISC_ERROR, 'Coin not found.');

          tx.view.addOutput(input.prevout, coin);
        }
      }
    }

    // Add all keys the wallet can find for its own coins
    // and whatever it can find for the coins added from the chain
    for (const key of await this.wallet.deriveInputs(tx))
      keys.push(key);
