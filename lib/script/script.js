...
class Script {
  ...
  execute(stack, flags, tx, index, value, digest) {
    if (flags == null)
  ...
  static verify(tx, index, coins, flags) {
    ...
    if ((flags & Script.flags.VERIFY_WITNESS) && output.isProgram()) {
      ...
      Script.verifyProgram(witness, output, flags, tx, index, coins);
    ...
    if ((flags & Script.flags.VERIFY_P2SH) && output.isScripthash()) {
      ...
      if ((flags & Script.flags.VERIFY_WITNESS) && redeem.isProgram()) {
        ...
        Script.verifyProgram(witness, redeem, flags, tx, index, coins);
  ...
  static verifyProgram(witness, output, flags, tx, index, coins) {
    const coin = coins[index];
    const value = coin.value;
    ...
    redeem.execute(stack, flags, tx, index, value, digests.WITNESSV0);
