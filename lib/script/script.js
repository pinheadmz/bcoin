...
class Script {
   ...
   * @param {Object?} sigopBudget - validation weight remaining (Tapscript).
   * @throws {ScriptError} Will be thrown on VERIFY failures.
   */

  execute(stack, flags, tx, index, value, digest, coins, sigopBudget) {
    if (flags == null)
    ...
    if (sigopBudget == null && digest === digests.TAPSCRIPT)
      throw new Error('Tapscript execution requires sigopBudget');

    // Different resource limits depending on type
      ...
      switch (op.value) {
        ...
        case opcodes.OP_NOTIF: {
          ...
          if (!negate) {
            ...
            // The MINIMALIF rules, which are only a standardness rule in P2WSH,
            // are consensus enforced in Tapscript.
            if (digest === digests.TAPSCRIPT ||
        ...
        case opcodes.OP_CHECKSIGADD: {
          ...
          const res = evalChecksigTapscript(
            tx,
            key,
            sig,
            sighashData,
  ...
  static verifyProgram(witness, output, flags, tx, index, coins, p2sh = false) {
    ...
    let redeem;
    let digest = null;
    // Must be object to pass by reference later
    const sigopBudget = {remaining: null};
    ...
    if (program.version === 0) {
      ...
      if (witness.getAnnex()) {
        if (flags & Script.flags.VERIFY_DISCOURAGE_UNKNOWN_ANNEX)
          throw new ScriptError('DISCOURAGE_UNKNOWN_ANNEX');
      ...
      if (stack.length === 1) {
        ...
        if (leafVersion === 0xc0) {
          ...
          // sipa/bitcoin#981be1f3b229830470823b5b0a4b078064b7b461
    ...
    assert(digest != null);
    redeem.execute(stack, flags, tx, index, value, digest, coins, sigopBudget);
  ...
  static verifyTaprootCommitment(witness, output) {
    ...
    return Schnorr.publicKeyTweakTest(internalKey, t, sign, outputKey);
  }
 ...
 * @param {Number} flags
 * @param {Object} sigopBudget
 * @returns {Boolean}
 */

function evalChecksigTapscript(tx, key, sig, sighashData, flags, sigopBudget) {
  const empty = sig.length === 0;
  if (!empty) {
    // Pay for this sigop and check the remaining budget in the gas tank
    assert(sigopBudget);
    assert(sigopBudget.remaining !== null);
    sigopBudget.remaining -= consensus.VALIDATION_WEIGHT_PER_SIGOP_PASSED;
    if (sigopBudget.remaining < 0)
  ...
  if (key.length === 0) {
    ...
    // Empty sig returns false but doesn't throw an error
    // (CHECKSIG will push a false on the stack and continue)
    if (empty)
      return false;
