      ...
      switch (op.value) {
        ...
        case opcodes.OP_CHECKSIGVERIFY: {
          ...
          if (digest !== digests.TAPSCRIPT) {
            ...
            res = evalChecksigTapscript(tx, key, sig, sighashData, flags);
          }
        ...
        case opcodes.OP_CHECKSIGADD: {
          ...
          const res = evalChecksigTapscript(tx, key, sig, sighashData, flags);
...
function evalChecksigTapscript(tx, key, sig, sighashData, flags) {
  const empty = sig.length === 0;
  if (!empty) {
    // Check the sigops gas tank
