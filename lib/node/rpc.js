...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('signrawtransactionwithkey', this.signRawTransactionWithKey);
  ...
  async getBlockchainInfo(args, help) {
    ...
    return {
      ...
      initialblockdownload: !this.chain.synced,
  ...
  async getBlock(args, help) {
    if (help || args.length < 1 || args.length > 2)
      throw new RPCError(errs.MISC_ERROR, 'getblock "hash" ( verbosity )');

    const valid = new Validator(args);
    const hash = valid.brhash(0);
    const verbosity = valid.u32(1, 1);
    ...
    return await this.blockToJSON(entry, block, verbosity);
  }
  ...
  async createRawTransaction(args, help) {
    ...
    for (const item of sendTo) {
      ...
      if (keys.length !== 1)
        throw new RPCError(errs.TYPE_ERROR, 'Invalid sendTo.');
      ...
      const send = new Validator(item);
      const value = send.ufixed(key, 8);
  ...
  async sendRawTransaction(args, help) {
    ...
    try {
      ...
      throw new RPCError(errs.VERIFY_REJECTED, e.reason);
    }
  ...
  async signRawTransaction(args, help) {
    if (help || args.length < 1 || args.length > 4) {
      throw new RPCError(errs.MISC_ERROR,
        ...
        + '"redeemScript":"hex","amount":n},...] ["privatekey1",...]'
        + ' sighashtype )');
    }
  ...
  async signRawTransactionWithKey(args, help) {
    if (help || args.length < 2 || args.length > 4) {
      throw new RPCError(errs.MISC_ERROR,
        ...
        + ' ["privatekey1",...] ('
        + ' [{"txid":"id","vout":n,"scriptPubKey":"hex",'
        + '"redeemScript":"hex","witnessScript":"hex","amount":n},...]'
        + ' sighashtype )');
    }

    const valid = new Validator(args);
    const data = valid.buf(0);
    const secrets = valid.array(1);
    const prevout = valid.array(2);
    const sighash = valid.str(3);
    ...
    const valid2 = new Validator(secrets);
    for (let i = 0; i < secrets.length; i++) {
      const secret = valid2.str(i, '');
      const key = parseSecret(secret, this.network);
      map.set(key.getPublicKey(), key);
      keys.push(key);
    }
    ...
    if (prevout) {
      for (const prev of prevout) {
        ...
        const value = valid.ufixed('amount', 8);
        const redeemRaw = valid.buf('redeemScript');
        const witnessRaw = valid.buf('witnessScript');

        if (!hash || index == null || !scriptRaw)
          throw new RPCError(errs.INVALID_PARAMETER, 'Invalid UTXO.');
        ...
        // "amount" is only required for SegWit spends and will be
        // checked later. So this is a hack just to get us a Coin object
        const coin = Output.fromScript(script, value || 0);
        ...
        if (script.isWitnessScripthash()) {
          if (!value) {
            throw new RPCError(errs.INVALID_PARAMETER,
              'SegWit inputs require amount.');
          }

          if (!witnessRaw) {
            throw new RPCError(errs.INVALID_PARAMETER,
              'P2WSH or P2SH-P2WSH requires witness script.');
          }

          redeem = Script.fromRaw(witnessRaw);
        }
  ...
  async testmempoolaccept(args, help) {
    ...
    const valid = new Validator(args);
    const items = valid.array(0);

    if (!items || items.length === 0)
      throw new RPCError(errs.TYPE_ERROR, 'Invalid TXs.');

    const response = [];
    const txs = new Validator(items);

    for (let i = 0; i < items.length; i++) {
      let tx = txs.buf(i);

      if (!tx)
        throw new RPCError(errs.TYPE_ERROR, 'Invalid TX.');
      ...
      let accept = false;
      try {
        accept = await this.mempool.testTX(tx);
      } catch(e) {
        err = e.message;
      }
  ...
  async blockToJSON(entry, block, verbosity) {
    const mtp = await this.chain.getMedianTime(entry);
    const next = await this.chain.getNextHash(entry.hash);
    const txs = [];
