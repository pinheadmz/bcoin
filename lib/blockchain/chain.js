...
class Chain extends AsyncEmitter {
  ...
  async getDeployments(time, prev) {
    ...
    // Check the state of Taproot deployment
    const taproot = await this.getState(prev, deployments.taproot);
    if (taproot === thresholdStates.ACTIVE)
      state.flags |= Script.flags.VERIFY_TAPROOT;
  ...
  setDeploymentState(state) {
    ...
    if (!this.state.hasTaproot() && state.hasTaproot())
      this.logger.warning('Taproot has been activated.');
...
class DeploymentState {
  ...
  /**
   * Test whether Taproot is active.
   * @returns {Boolean}
   */

  hasTaproot() {
    return (this.flags & Script.flags.VERIFY_TAPROOT) !== 0;
  }
}
