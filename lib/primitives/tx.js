...
const {encoding} = bio;
const {hashType, digests, isTaprootHashType} = Script;
const {inspectSymbol, taggedHash} = require('../utils');
...
class TX {
   ...
   * @param {Number} digest - Transaction serialization digest type.
   * @param {Coin[]?} coins - Array of all input coins (required for Taproot)
   * @param {Number?} codeseparator_position (required for Tapscript)
   * @returns {Buffer} Signature hash.
   */

  signatureHash(index, prev, value, type, digest, coins, pos) {
    ...
    if (digest === digests.LEGACY)
      return this.signatureHashLegacy(index, prev, type);

    // Segwit V0 sighashing
    if (digest === digests.WITNESSV0)
      return this.signatureHashWitnessV0(index, prev, value, type);

    // Segwit V1 (Taproot) sighashing
    if (digest === digests.TAPROOT || digest === digests.TAPSCRIPT) {
      assert(Array.isArray(coins));
      return this.signatureHashTaproot(index, value, type, coins, pos);
    }
  ...
  signatureHashLegacy(index, prev, type) {
  ...
  signatureHashWitnessV0(index, prev, value, type) {
    const input = this.inputs[index];
  ...
  /**
   * WitnessV1 (Taproot) sighashing
   * @private
   * @param {Number} index
   * @param {Amount} value
   * @param {SighashType} type
   * @param {Coin[]} coins
   * @param {Number?} pos - codeseparator_position (Required for Tapscript)
   * @returns {Buffer}
   */

  signatureHashTaproot(index, value, type, coins, pos) {
    if (!isTaprootHashType(type))
      throw new ScriptError('TAPROOT_INVALID_SIG');

    const input = this.inputs[index];
    const coin = coins[index];
    const scriptPubKey = coin.script;
    let prevouts = null;
    let amounts = null;
    let sequences = null;
    let outputs = null;
    ...
    if (!(type & hashType.ANYONECANPAY)) {
      if (this._hashPrevouts) {
        ...
        prevouts = sha256.digest(bw.render());
      ...
      if (this._hashAmounts) {
        amounts = this._hashAmounts;
      } else {
        const bw = bio.pool(this.inputs.length * 8);

        for (const coin of coins)
          bw.writeU64(coin.value);

        amounts = sha256.digest(bw.render());

        if (!this.mutable)
          this._hashAmounts = amounts;
      }
      ...
      if (this._hashSequence) {
        ...
        sequences = sha256.digest(bw.render());

        if (!this.mutable)
          this._hashSequence = sequences;
      }

      // Three hash comitments above
      txDataSize += 32 * 3;

      // Also commit to this input's index in the TX
      txDataSize += 4;
    } else {
      // ANYONECANPAY commits only to this input's outpoint / amount / sequence
      txDataSize += 48;
    }
    ...
    if ((type & 0x1f) !== hashType.SINGLE
      ...
      if (this._hashOutputs) {
        ...
        outputs = sha256.digest(bw.render());
    ...
    // Taproot spend_type:
    // has annex? =>    00000001
    // is tapscript? => 00000010
    const spendType = input.witness.getSpendType();

    let size = 0;
    // epoch, type, version, locktime, spendtype
    size += 11;
    // input and output commitments based on sighash type
    size += txDataSize;
    // output script being spent
    size += scriptPubKey.getVarSize();
    // annex hash
    if (spendType & (1 << 0))
      size += 32;
    // sha_single_output
    if ((type & 0x1f) === hashType.SINGLE)
      size += 32;
    // tapscript hash, key version, codeseparator position
    if (spendType & (1 << 1))
      size += 37;
    ...
    bw.writeU8(spendType);
    bw.writeVarBytes(scriptPubKey.toRaw());
    ...
    if (spendType & (1 << 0)) {
      const annex = input.witness.getAnnex();
      const size = encoding.sizeVarBytes(annex);
      const serializedAnnex = bio.write(size);

      serializedAnnex.writeVarBytes(annex);
      bw.writeHash(sha256.digest(serializedAnnex.render()));
    }

    if ((type & 0x1f) === hashType.SINGLE) {
      assert(index < this.outputs.length, 'Input index exceeds outputs.');
      const output = this.outputs[index];
      const hashSingleOutput = sha256.digest(output.toRaw());
      bw.writeBytes(hashSingleOutput);
    }

    if (spendType & (1 << 1)) {
      const controlBlock = input.witness.getControlBlock();
      const leafVersion = controlBlock[0] & 0xfe;

      const tapleaf = input.witness.getTapleaf();
      const size = encoding.sizeVarBytes(tapleaf);
      const serializedTapleaf = bio.write(size + 1);

      serializedTapleaf.writeU8(leafVersion);
      serializedTapleaf.writeVarBytes(tapleaf);
      bw.writeHash(taggedHash.TapLeafHash.digest(serializedTapleaf.render()));

      // key_version "a constant value 0x00"
      bw.writeU8(0);
    ...
    return taggedHash.TapSighashHash.digest(bw.render());
  }
  ...
  checksig(index, prev, value, sig, key, digest) {
    if (sig.length === 0)
      return false;

    const type = sig[sig.length - 1];
    const hash = this.signatureHash(index, prev, value, type, digest);
  ...
  signature(index, prev, value, key, type, digest) {
    if (type == null)
      type = hashType.ALL;
   ...
   * verified.
   * @param {Coin[]|Output[]} coins - Previous outputs.
   * @param {VerifyFlags} [flags=STANDARD_VERIFY_FLAGS]
   * @throws {ScriptError} on invalid input
   */

  checkInput(index, coins, flags) {
    assert(this.inputs[index], 'Input does not exist.');
    assert(coins[index], 'Coin does not exist.');
  ...
  async checkInputAsync(index, coins, flags, pool) {
    assert(this.inputs[index], 'Input does not exist.');
    assert(coins[index], 'Coin does not exist.');

    if (!pool) {
      this.checkInput(index, coins, flags);
      return;
    }

    await pool.checkInput(this, index, coins, flags);
  }
  ...
  async verifyInputAsync(index, coins, flags, pool) {
    try {
      await this.checkInput(index, coins, flags, pool);
    } catch (e) {
      if (e.type === 'ScriptError')
        return false;
      throw e;
  ...
  hasStandardWitness(view) {
    ...
    for (const input of this.inputs) {
      ...
      if (prev.isPayToTaproot() && !coin.script.isScripthash()) {
        let ignore = 0;

        // Taproot witness: ignore annex
        if (witness.getAnnex())
          ignore++;

        // Taproot witness with zero items (besides annex) is invalid
        if (witness.items.length - ignore === 0)
          return false;

        // Empty control block is invalid
        const controlBlock = witness.getControlBlock();
        if (controlBlock && controlBlock.length === 0)
          return false;

        // Tapscript witness: ignore control block
        if (controlBlock && ((controlBlock[0] & 0xfe) === 0xc0)) {
          ignore++;

          // Check size of remaining stack items
          // (annex, controlBlock and script are ignored)
          for (let i = 0; i < witness.items.length - ignore - 1; i++) {
            const item = witness.items[i];
            if (item.length > policy.MAX_P2WSH_PUSH)
              return false;
          }
        ...
        // No other standardness rules apply to Taproot witnesses
        continue;
      }
